...
import { assertMachOArchitecture, readMachOArchitecture } from '../src/mach-o.ts';
...
test('reads the Apple arm64 and x86_64 Mach-O CPU types', async () => {
  await expect(readMachOArchitecture(await machOFixture(0x0100000c))).resolves.toBe('arm64');
  await expect(readMachOArchitecture(await machOFixture(0x01000007))).resolves.toBe('x64');
});
...
test('retains the filesystem cause for a missing image', async () => {
  ...
  try {
    await readMachOArchitecture(path);
  } catch (value) {
...
test('rejects a non-Mach-O image with its parser cause intact', async () => {
  ...
  expect(error.message).toContain(path);
  expect(error.cause).toMatchObject({ message: 'the file is not a thin 64-bit Mach-O image' });
});
