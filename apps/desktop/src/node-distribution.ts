...
import { architectureForTargetTriple, type DesktopTargetTriple } from './release-contract.ts';
...
// Node.js publishes these target-owned archive hashes in the immutable release
// checksum inventory. Updating .node-version therefore requires updating this
// owning association rather than accepting a same-version archive for another CPU.
...
export const acquireNodeDistribution = async (
  ...
  if (!response.ok) {
    throw new Error(`Node.js archive ${distribution.archive} returned HTTP ${response.status}`);
  }
  const archive = Buffer.from(await response.arrayBuffer());
  const actualHash = createHash('sha256').update(archive).digest('hex');
  if (actualHash !== distribution.sha256) {
    throw new Error(
      `Node.js archive ${distribution.archive} has SHA-256 ${actualHash}; expected ${distribution.sha256}`,
    );
  }
