...
首次启动面向已经装有可用 Codex 或 Claude Code 的 owner，不面向零基础用户。缺少可用 Agent 时，界面只说明此前提，不展开 Agent 安装或使用教学。

Quick Start 将激活呈现为一个由真实状态驱动的任务：界面一次只展示一个当前目标，每个检查点都从可观察的 Gateway 或本机客户端状态推导，而不是持久化的向导步骤。窗口隐藏、刷新、重启、重试，以及在页面外完成的工作，都不会虚构或丢失进度。

激活检查点依次为：

1. 本机 Gateway 健康。
2. Floway Skill 已授权（安装一次，Codex 与 Claude Code 共用）。
3. 存在一个已启用且可列出模型的 Upstream。
4. 存在至少一个可用 API Key。
5. 所选客户端的 Agent Setup 已就绪（客户端配置指向本机 Gateway）。
6. 从该客户端的新会话完成一次成功请求。

默认路径是让 owner 已有的 Agent 通过 Floway Skill 完成配置与验证：每个配置类目标给出可直接发给 Agent 的请求，同时保留对应的既有 Dashboard 管理页面作为手动入口。激活不得破坏用于引导的现有 Agent：在 Floway 模型路径验证通过、客户端 Setup 就绪之前，其原有配置保持可用；失败的 Setup 不得让 owner 失去引导用的 Agent。

各类失败（网络、Provider、模型发现、API Key、客户端 Setup、验证）保持可重试，保留原始诊断信息，且绝不显示为已完成。

owner 可以显式跳过 Quick Start 进入运行概览，跳过不视为激活成功；Quick Start 可随时从导航重新打开。全部检查点完成后进入运行概览，引导界面不成为产品的日常管理界面。
